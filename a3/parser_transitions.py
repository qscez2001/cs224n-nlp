...
def minibatch_parse(sentences, model, batch_size):
    ...
    #   Use the model to predict the next transition for each partial parse in the minibatch 
        pre_trans = model.predict(unfinished_parses[:batch_size])
    #   Perform a parse step on each partial parse in the minibatch with its predicted transition 
        for pre_tran, partial_parse in zip(pre_trans, unfinished_parses[:batch_size]):
            # print(pre_tran, partial_parse)
            partial_parse.parse_step(pre_tran)
...
def test_step(name, transition, stack, buf, deps,
    ...
    # print(pp.stack, pp.buffer, pp.dependencies)
