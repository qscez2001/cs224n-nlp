...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        if type(sents[0]) == list:
            result = [[[self.char2id[c] for c in w] for w in s] for s in sents]
            
            for sent in result:
                for word in sent:
                    word.insert(0, self.start_of_word)
                    word.append(self.end_of_word)
        else:
            result = [[self.char2id[c] for c in w] for w in sents]
            for word in result:
                word.insert(0, self.start_of_word)
                word.append(self.end_of_word)
        # print(result)
