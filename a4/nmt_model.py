...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # 2*5*3 => 5*6
        concated_last_hidden = torch.cat((last_hidden[0],last_hidden[1]),1)
        ###             Apply the h_projection layer to this in order to compute init_decoder_hidden.
        init_decoder_hidden = self.h_projection(concated_last_hidden)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        ###     3. Compute the attention scores e_t, a Tensor shape (b, src_len). (5, )
        ###        Note: b = batch_size, src_len = maximum source length, h = hidden size.
        # print(dec_hidden.size()) 5*3
        # print(enc_hiddens_proj.size()) 5*20*3
        # e_t: 5*20
        # print(torch.unsqueeze(dec_hidden, 2).size()) squeeze多一個維度 5*3*1
        # 3維*3維 = 3維, 但我們想要2維，所以要再unsqueeze第三個維度
        ...
        # print(torch.unsqueeze(alpha_t, 1).size())
        a_t = torch.squeeze(torch.bmm(torch.unsqueeze(alpha_t, 1), enc_hiddens),1)
        #$$     Hints:
        ###           - alpha_t is shape (b, src_len) (5,20) =>(5,1,20)
        ###           - enc_hiddens is shape (b, src_len, 2h) (5,20,6)
        ###           - a_t should be shape (b, 2h) (5,6) <= (5,20,6)
